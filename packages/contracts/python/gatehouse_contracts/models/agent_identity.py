...
class DeclaredCapability(RootModel[str]):
...
class Provenance(BaseModel):
    ...
    operator: Annotated[
        str,
        Field(
            description='The operating authority or person behind the agent (DID or display string).'
        ),
    ]
    origin: Annotated[
        str | None,
        Field(description='Where the agent comes from (stack, vendor, repo).'),
    ] = None
    attestations: Annotated[
        list[Sha256Hex] | None,
        Field(description='Content hashes of prior attestation documents, if any.'),
    ] = None
...
class AgentIdentity(BaseModel):
    ...
    id: Did
    publicKeyMultibase: Annotated[str, Field(min_length=1)]
    declaredCapabilities: Annotated[list[DeclaredCapability], Field(min_length=1)]
